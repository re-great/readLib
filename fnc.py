...
def searchResults(query, frame, k):
    if k == 1:
        res = GetbyTitle(query)
    ...
    canvas = Canvas(frame, bg='brown', bd=1)
    canvas.place(relx=0, rely=0, relheight=1, relwidth=1)
    ...
    for i in range(1, len(res)):
        ...
        try:
            ...
            label1 = Label(lf, text=f'Title : {TextTitle}\n', font=("Courier", 18), bg='black', fg='white',
                           wraplength=w, justify="center")
            label2 = Label(lf, text=f'Author : {Author}\n', font=("Courier", 18), bg='black', fg='white', wraplength=w,
                           justify="center")
            label3 = Label(lf, text=f'Page Count : {TextPages}\n', font=("Courier", 18), bg='black', fg='white',
                           wraplength=w, justify="center")
            label4 = Label(lf, text=f'Language: {TextLang}\n', font=("Courier", 18), bg='black', fg='white',
                           wraplength=w, justify="center")
            button = Button(lf, text="Download NOW!!!", bg='green', font=("Courier", 12), fg='red', justify="center",
                            pady=10, padx=10, command=lambda: Redirect(Link))
...
def createResult(newRoot):
    ...
    t1 = """Search result will be displayed here
		"""
...
def createSearch(newRoot):
    options = ['By Book Name', 'By Author']
    query = StringVar()
    print("HERE\n")
    ...
    fl_b1 = Button(SearchFrame, text='Search', bg='black', fg='orange',
                   command=lambda: searchResults(fl_e1.get(), Result, 1))

    fl_l2 = Label(SearchFrame, text=options[1], bg='black', fg='orange')
    fl_e2 = Entry(SearchFrame)
    fl_b2 = Button(SearchFrame, text='Search', bg='black', fg='orange',
                   command=lambda: searchResults(fl_e2.get(), Result, 0))
